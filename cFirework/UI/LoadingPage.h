...
public:
	...
	LoadingPage() {
		...
		this->setAttribute(Qt::WA_TranslucentBackground);
		CentralWidget = new QFrame(this);
		CentralWidget->setStyleSheet("QFrame{background-color:#00000000;border-radius:10px;border-image:url(':/IRC/Icon/LoadingPage.png');}");
		TitleLabel = new QLabel(this);
		TitleLabel->setText("FireWork");
		NoticeLabel = new QLabel(this);
	...
	void resizeEvent(QResizeEvent* event = Q_NULLPTR) {
		...
		TitleLabel->setGeometry(QRect(width() * 0.011, height() * 0.4, width() * 0.5, height() * 0.2));
		NoticeLabel->setGeometry(QRect(width() * 0.011, height() * 0.93, width() * 0.5, height() * 0.05));
		TitleLabel->setStyleSheet("QLabel{color:#FFFFFF;font-family:'Microsoft YaHei';font-size:" + QString::number((int)(height() * 0.18)) + "px}");
		NoticeLabel->setStyleSheet("QLabel{color:#888888;font-family:'Microsoft YaHei';font-size:" + QString::number((int)(height() * 0.05)) + "px}");
	}
public slots:
	void checking(void) {
